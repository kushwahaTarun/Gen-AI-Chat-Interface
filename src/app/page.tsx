...
export default function Home() {
  ...
  // Handle speak for specific message
  const handleSpeakMessage = (
    messageContent: string,
    messageId: string | number
  ) => {
    // Create a callback that updates the specific message's speaking state
    const onSpeechStateChange = (isSpeaking: boolean) => {
      if (isSpeaking) {
    ...
    handleSpeak(messageContent, onSpeechStateChange);
  };

  // Handle pause for specific message
  const handlePauseMessage = () => {
    const onSpeechStateChange = (isSpeaking: boolean) => {
      setSpeakingMessageId(null);
    };

    handleTextToSpeechPause(onSpeechStateChange);
  };
                        ...
                        >
                          ...
                          <div className={`flex items-center justify-end space-x-0.5 w-1/2 ${
                            ...
                            <span
                              ...
                              className="hover:bg-gray-800 text-gray-400 hover:text-white p-1.5 rounded"
                            >
                              {/* Check if THIS specific message is currently speaking */}
                              {!isCurrentlySpeaking ? (
                                <RxSpeakerLoud
                                  className="cursor-pointer text-base"
                                  onClick={() =>
